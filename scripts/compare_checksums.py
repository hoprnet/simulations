...
def main(leftfile: str, rightfile: str):
    leftdata, rightdata = import_files(leftfile, rightfile)

    l_striped = [StrippedLine(left) for left in leftdata]
    r_striped = [StrippedLine(right) for right in rightdata]
    ...
    l_start, r_start = l_blocks.index(first_shared_block), r_blocks.index(
        first_shared_block
    )
    idx = (
        sum(
            [
                left == right
                for left, right in zip(l_striped[l_start:], r_striped[r_start:])
            ]
        )
        - 1
    )
    ...
    assert (
        r_striped[r_start + idx] == l_striped[l_start + idx]
    ), "Last common block is not the same"
    assert (
        r_striped[r_start + idx + 1] != l_striped[l_start + idx + 1]
    ), "Next block is the same"
